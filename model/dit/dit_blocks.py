...
class LightDiTHead(nn.Module):
    ...
    def __init__(self,n_layers,d_model,num_attention_heads,attention_head_dim,num_codebooks):
        super().__init__()
        self.pos_embed = FluxPosEmbed(theta=10000, axes_dim=[16,56,56])
        self.blocks = nn.ModuleList([
            SingleTransformerBlock(d_model,num_attention_heads=num_attention_heads,attention_head_dim=attention_head_dim)
            for _ in range(n_layers)
        ])
        self.time_text_embed = TimestepProjEmbeddings(
            embedding_dim=d_model
        )
        self.norm_out = AdaLayerNormContinuous(d_model,d_model, elementwise_affine=False, eps=1e-6)
        self.proj_out = nn.Linear(d_model, num_codebooks, bias=True)
        self.norm_in = AdaLayerNormContinuous(d_model,d_model, elementwise_affine=False, eps=1e-6)
        self.proj_in = nn.Linear(d_model, d_model, bias=True)
    ...
    def forward(self,x,timesteps):
        ...
        b,l,d = x.shape
        _asp = int(np.sqrt(l))
        ids = self._prepare_latent_image_ids(b, _asp, _asp, x.device, x.dtype)
        image_rope = self.pos_embed(ids)
        
        temb = self.time_text_embed(timesteps,x)
        x = self.norm_in(x,temb)
        x = self.proj_in(x)
        for block in self.blocks:
            # breakpoint()
            if self.training:
                x = deepspeed.checkpointing.checkpoint(block, x, temb, image_rope)
            else:
                x = block(x,temb,image_rope)
        x = self.norm_out(x,temb)
        x = self.proj_out(x)
        return x

if __name__ == '__main__':
    model = LightDiTHead(n_layers=38,d_model=2048,num_attention_heads=24,attention_head_dim=128,num_codebooks=8192).cuda()
    # 38 layer = 2.4B
    # 20 layer = 1.2B
    model.eval() # 1.2 B
    num_params = sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e9
    print(f"Number of trainable parameters: {num_params}B")
    data = torch.rand(2,4096,2048).cuda()
    timesteps = torch.rand(2).cuda()
    y = model(data,timesteps)
    breakpoint()
