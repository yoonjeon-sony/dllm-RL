...
# from .misc import *
# import common
from llava.model.multimodal_encoder.magvit2.common import *
from llava.model.multimodal_encoder.magvit2.utils import ModelMixin,ConfigMixin,register_to_config
import math
...
def image_transform(image, normalize=True):
...
if __name__ == '__main__':
    ...
    def pad_to_square_and_resize(img, size=1024):
        ...
        return resized_img
    model = MAGVITv2.from_pretrained('/mnt/localssd/magvitv2',low_cpu_mem_usage=False)
    model.cuda()
    #breakpoint()
    # .cuda()
    model.requires_grad_(False)
    processor = MagVitImageProcessor()
    image = Image.open('images/anime.png')
    # image = image.resize((1024,1024))
    image = pad_to_square_and_resize(image,1024)
    #image = image.pad
    image_tensor = processor.preprocess(image).cuda()
    codes = model.get_code(image_tensor.repeat(2,1,1,1)) # 1, 4096
    # z = torch.rand_like(codes,dtype=torch.float)
    # mask = z < 0.1
    # rand_code = torch.randint_like(codes,8192)
    # code_masked = torch.where(mask,rand_code,codes)
    image_rec = model.decode_code(codes)
    image_rec = torch.clamp((image_rec + 1.0) / 2.0, min=0.0, max=1.0)
    image_rec *= 255.0
    image_rec = image_rec.permute(0, 2, 3, 1).cpu().numpy().astype(np.uint8)
    Image.fromarray(image_rec[0]).save('a.jpg')
