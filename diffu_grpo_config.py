...
class DiffuGRPOConfig(GRPOConfig):
    ...
    image_logit_sample_chunk_size: int = field(
        default=1,
        metadata={
            "help": "Number of samples to process at once when projecting image hidden states to image logits during PPO logprob computation. Lower values reduce peak memory."
        },
    )
    ...
    completion_log_sample_ratio: float = field(
        default=0.1,
        metadata={"help": "Fraction of completion rows to sample when logging prompts, completions, and rewards."},
    )
    image_log_sample_ratio: float = field(
        default=0.1,
        metadata={"help": "Fraction of image_gen rows to include with original and edited images in wandb logs."},
    )
    image_log_steps: int = field(
        default=50,
        metadata={"help": "Log original and edited images to wandb every `image_log_steps` steps."},
    )
    ...
    dynamic_threshold: Optional[float] = field(
        default=0.90,
        metadata={"help": "Dynamic threshold for remasking."},
    )
    ...
    # Interleaved generation settings
    mode: Optional[str] = field(
        default="interleaved",
        metadata={"help": "Mode for interleaved generation. 'interleaved' or 'interleaved_region'."},
    )
    do_plan: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to generate a planning step first."},
    )
    conf_policy: Optional[str] = field(
        default="mmada",
        metadata={"help": "Confidence policy for interleaved generation. 'mmada', 'mask_git', 'stratified'."},
    )
    enable_stratified: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to enable stratified sampling."},
    )
    edit_mode: Optional[int] = field(
        default=0,
        metadata={"help": "Edit mode for interleaved generation. 0, 1, 2, 3 -> different unconditional types."},
    )
    guidance_scale: Optional[float] = field(
        default=1.2,
        metadata={"help": "CFG scale for text conditioning."},
    )
    guidance_scale_image: Optional[float] = field(
        default=1.4,
        metadata={"help": "CFG scale for image conditioning."},
    )
    rotatedDiff_key: Optional[str] = field(
        default="rotatedDiff",
        metadata={"help": "Key in data for rotated rect (for interleaved_region mode)."},
    )

    data_root: Optional[str] = field(
        default="/home/yoonjeon.kim/dLLM-RL/data",
        metadata={"help": "Root directory for ThinkMorph jsonl files."},
    )
    ...
    eval_tasks: Optional[str] = field(
        default=None,
        metadata={
            "help": "Comma-separated lmms-eval task names, e.g. "
            "'chartqa,blink_jigsaw,cv_bench,vstar_bench,VisualPuzzles_direct'"
        },
    )
    eval_parquet_dir: Optional[str] = field(
        default=None,
        metadata={
            "help": "Dir with {task_name}_n100_seed42.parquet files from prepare_eval_datasets.py"
        },
    )
    eval_max_new_tokens_override: Optional[int] = field(
        default=64,
        metadata={
            "help": "Cap max_new_tokens for all eval tasks (VisualPuzzles default=4096 is too slow). "
